...
// The proposal as a client reads it — scope, terms, investment, what happens
// next, and the decision. Two pages render it: the public /p/:token page (a
// prospect with no login, token in a header) and the signed-in portal's
// /agreements/proposal-:id (same view, scoped to their account). The caller
// supplies the three API calls; this component owns everything else.

const props = withDefaults(defineProps<{
  variant: 'public' | 'portal'
  fetchView: () => Promise<View>
  onAccept: (name: string) => Promise<{ contract_id?: number | null } | undefined>
  onDecline: (reason: string) => Promise<unknown>
  defaultName?: string
}>(), { defaultName: '' })
const emit = defineEmits<{ decided: [outcome: 'accepted' | 'declined', contractId: number | null] }>()
...
const yourName = ref(props.defaultName)
...
async function load() {
  try {
    const data = await props.fetchView()
    proposal.value = data
    if (data.decided) {
      outcome.value = (data.status === 'accepted' ? 'accepted' : 'declined')
      contractId.value = data.contract_id ?? null
    }
  } catch (err: unknown) {
...
onMounted(load)
defineExpose({ title: computed(() => proposal.value?.title ?? null) })

async function accept() {
  busy.value = 'accept'
  try {
    const res = await props.onAccept(yourName.value)
    contractId.value = res?.contract_id ?? null
    outcome.value = 'accepted'
    emit('decided', 'accepted', contractId.value)
  } catch (err: unknown) {
    loadError.value = (err as { data?: { error?: { message?: string } } })?.data?.error?.message
...
async function decline() {
  busy.value = 'decline'
  try {
    await props.onDecline(declineReason.value)
    outcome.value = 'declined'
    emit('decided', 'declined', null)
  } catch (err: unknown) {
    loadError.value = (err as { data?: { error?: { message?: string } } })?.data?.error?.message
      || 'Something went wrong. Please get in touch.'
  } finally { busy.value = null }
...
<template>
  <div
    ...
    :class="variant === 'public' ? 'min-h-screen' : 'overflow-hidden rounded-band ring ring-default'"
  >
    <!-- ===== ink band: the one per page ===== -->
    <header class="bg-deep text-white">
      <div
        class="mx-auto w-full max-w-[880px] px-5 sm:px-8"
        :class="variant === 'public' ? 'py-8 sm:py-10' : 'py-7 sm:py-8'"
      >
        <div
          v-if="variant === 'public'"
        ...
        <div
          ...
          :class="variant === 'public' ? 'mt-12 sm:mt-16' : ''"
        >
          <div
    ...
    <main
      class="mx-auto w-full max-w-[880px] flex-1 px-5 sm:px-8"
      :class="variant === 'public' ? 'py-10 sm:py-14' : 'py-8 sm:py-10'"
    >
      <div
        v-if="pending"
      ...
      <template v-else-if="proposal">
        ...
        >
          ...
          <p class="mt-4 text-[15px] leading-relaxed text-muted">
            <template v-if="outcome === 'accepted' && contractId">
              Your agreement is ready. Sign it here in the portal or from the email we sent.
            </template>
            <template v-else-if="outcome === 'accepted'">
          ...
          <NuxtLink
            v-if="outcome === 'accepted' && contractId"
            :to="`/agreements/contract-${contractId}`"
            class="mt-6 inline-flex items-center rounded-btn bg-primary px-5 py-2.5 text-sm font-semibold text-inverted transition-colors hover:bg-primary/90"
          >
            Sign Agreement
          </NuxtLink>
          <p
          ...
          <p class="mt-10 text-center text-[13px] text-muted">
            ...
            <template v-if="agencyEmail">
              Email <a
                :href="`mailto:${agencyEmail}`"
                class="font-semibold text-highlighted underline decoration-ink-300 decoration-1 underline-offset-4 hover:decoration-citrine hover:decoration-2"
              >{{ agencyEmail }}</a>{{ variant === 'public' ? ' or just reply to the email this came in.' : ' or open a support ticket.' }}
            </template>
            <template v-else>
              {{ variant === 'public' ? 'Just reply to the email this came in.' : 'Open a support ticket and we’ll get back to you.' }}
            </template>
          </p>
        </template>
      </template>
