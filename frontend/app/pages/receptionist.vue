...
const canConvert = computed(() => !!cur.value && cur.value.type === 'inquiry' && !cur.value.contactId)
const curReviewed = computed(() => cur.value ? !cur.value.unread : false)
const curLink = computed(() => {
  if (!cur.value?.contactId) return null
  const isClient = cur.value.type === 'client'
  return {
    href: isClient ? `/clients/${cur.value.contactId}` : '/leads',
    label: cur.value.business || (isClient ? 'View client' : 'View lead'),
...
async function confirmConvert() {
  ...
  if (!c) return
  try {
    const { data } = await api<{ data: { contact: { id: number } } }>(`/calls/${c.id}/convert`, { method: 'POST' })
    c.contactId = data.contact.id
    toast.add({ title: 'Lead created', description: `${c.name ?? c.number} added to Leads → Inbound.`, color: 'success' })
  } catch {
