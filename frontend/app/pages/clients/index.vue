...
<template>
  ...
  <div class="flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="inline-flex items-center gap-0.5 rounded-[10px] border border-default bg-muted p-0.5">
      <button
        ...
        :class="tab === t.key ? 'bg-default font-semibold text-highlighted shadow-sm' : 'font-medium text-muted hover:text-highlighted'"
        @click="tab = t.key"
      >
        {{ t.label }}
        <span
          class="rounded-full px-1.5 py-px text-[11px] font-semibold tabular-nums"
          :class="tab === t.key ? 'bg-mist text-teal-700' : 'bg-elevated text-muted'"
        >{{ t.count }}</span>
      </button>
    </div>
  ...
  <div class="overflow-hidden rounded-card bg-default ring ring-default">
    ...
    <div v-else class="flex flex-col items-center px-6 py-20 text-center">
      <span class="mb-4 inline-flex size-14 items-center justify-center rounded-[14px] bg-muted text-muted">
        <UIcon name="i-lucide-search" class="size-6" />
      </span>
      <h3 class="font-display text-xl font-medium text-highlighted">No clients match</h3>
